...
- [x] Linux AppImage / Flatpak の要否判断 (2026-04-25): **不採用** で確定。判断根拠と再評価条件を [docs/distribution-formats.md](../distribution-formats.md) に記録。要点: ① ユーザー要望ゼロ、② Flatpak は keyring / Vulkan / IPC loopback と sandbox の衝突コストが高い、③ AppImage は FUSE 依存と glibc 互換が tar.gz と本質的に変わらない。
