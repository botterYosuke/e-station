---
title: nautilus_trader データマッピング (外部 API / データ型対応表)
status: migrated
migrated_from:
  - docs/✅nautilus_trader/data-mapping.md
...
| `Minute` | `equities_bars_minute_{YYYYMMDD}.csv.gz` を期間に重なる月だけ順次 stream-open（N1.2 訂正済み） |
