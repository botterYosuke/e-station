...
（フッター）**・**File メニュー（Open / Save / Save As）**・**モード切替**の実装仕様。
...
- [`archive/wandb-submit-impl.md`](./archive/wandb-submit-impl.md) — **(歴史)** Tools サブメニューによる W&B Submit 実装記録。W&B 機能移管（`🐃_blacksheep`）後に archive
...
- 抑制理由優先順位: `mode_switch_in_progress` > `engine_busy`
...
正規化される。menu 項目の集合計算（`actions_for_mode`）
...
| `src/menu_bar_state.rs` | widget bar 用の `TopMenu`（`File` 1 本立て） / `BarMessage` / `update`（全 OS） |
