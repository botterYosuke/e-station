---
title: 立花証券 e支店 API 統合プラン
status: migrated
migrated_from:
  - docs/✅tachibana/README.md
...
親計画は [docs/specs/data-engine/spec.md](../../specs/data-engine.md)（Rust ビュアー + Python データエンジン）。本計画はその上に「日本株（authenticated venue）」を載せるための差分仕様。
...
| ファイル | 役割 |
| :--- | :--- |
| [spec.md](../../specs/venues/tachibana.md) | ゴール・非ゴール・スコープ。何を作り何を作らないか |
| architecture.md | プロセス境界（Rust / Python）、認証クレデンシャルの所在、起動シーケンス |
| data-mapping.md | 立花のドメイン概念 ↔ 既存 IPC DTO のマッピング、新設 DTO・Venue・MarketKind |
| [implementation-plan.md](./implementation-plan.md) | フェーズ分割・受け入れ条件・テスト戦略 |
| implementation-plan-T3.5.md | T3.5（VenueState FSM / U4 Gate / U1 ログインボタン / U2 バナー / U3 auto-fire / U5 E2E）のステップ別計画 |
| invariant-tests.md | T35- prefix の不変条件テスト一覧（Hello/Ready/SetVenueCredentials/VenueReady シーケンス、resubscribe 単一性ほか） |
| review-fixes-2026-04-25.md | 2026-04-25 レビュー指摘の対応記録（H5 path fidelity / H6 keyring slot ほか） |
| review-fixes-2026-04-26.md | 2026-04-26 PlanLoop ラウンド 3 修正記録（行番号 → path::symbol 機械置換、用語使い分け、T35- prefix 必須化、アンカー死活修正） |
| review-fixes-tachibana-docs-2026-04-26.md | 2026-04-26 新セッション PlanLoop 修正記録（tachibana_ws.py 実装済み反映・インクリメンタル検索追記・invariant-tests エントリ追加・impl-plan B2/B5 整理ほか） |
| [open-questions.md](./open-questions.md) | 未確定事項と決定期限 |

## 一次資料

- 公式マニュアル: [.claude/skills/tachibana/manual_files/mfds_json_api_ref_text.html](https://github.com/botterYosuke/e-station/blob/main/.claude/skills/tachibana/manual_files/mfds_json_api_ref_text.html)
- REQUEST I/F PDF: [.claude/skills/tachibana/manual_files/api_request_if_v4r7.pdf](https://github.com/botterYosuke/e-station/blob/main/.claude/skills/tachibana/manual_files/api_request_if_v4r7.pdf)
- マスタ I/F PDF: [.claude/skills/tachibana/manual_files/api_request_if_master_v4r5.pdf](https://github.com/botterYosuke/e-station/blob/main/.claude/skills/tachibana/manual_files/api_request_if_master_v4r5.pdf)
- Python サンプル一式: [.claude/skills/tachibana/samples/](https://github.com/botterYosuke/e-station/blob/main/.claude/skills/tachibana/samples/)
...
- コーディング規約・運用ルール: [.claude/skills/tachibana/SKILL.md](https://github.com/botterYosuke/e-station/blob/main/.claude/skills/tachibana/SKILL.md)（**R1〜R10 を必ず守る**）
...
- **復旧シーケンスの source of truth は `ProcessManager`**。managed mode の再起動時は `Hello -> Ready -> SetProxy -> [Python 自律] startup_login -> VenueReady -> metadata fetch / resubscribe` を必ず再実行する（`SetVenueCredentials` は廃止済み。Python が自律的に `startup_login` を実行し `VenueReady` を送信する）。**T3.5 にて `VenueState` FSM / U4 Gate 着地済**: [src/venue_state.rs](https://github.com/botterYosuke/e-station/blob/main/../src/venue_state.rs) で FSM 化、[engine-client/src/process.rs](https://github.com/botterYosuke/e-station/blob/main/../engine-client/src/process.rs) の `apply_after_handshake` は SetProxy 後に `VenueReady` を非同期で待ち受け、受信後に resubscribe を発火する経路に更新済。U4 Gate により `VenueReady` 前の業務リクエストはブロックされる
- **`VenueReady` は冪等イベント**。Python が `startup_login` 完了ごとに送出する。UI は初回 / 再送を区別しない。Rust 側の resubscribe は `ProcessManager` 1 箇所に集約し、UI view 側は `VenueReady` イベントで新規 subscribe を発行しない
...
- ~~**`SetVenueCredentials` payload は typed**~~ — **削除済み**。`SetVenueCredentials` コマンドおよび `VenueCredentialsRefreshed` イベントは Python 自律ログイン方式（architecture.md §2）への移行で廃止された。クレデンシャルは Python が `tachibana_session.json` / `tachibana_account.json` で自前管理し、Rust は保持も送受信も行わない
- **第二暗証番号は Phase 1 では収集も保持もしない**（F-H5、Q11 改訂）。発注しないものを保持して攻撃面（コアダンプ・スワップ・GC 残存）を増やさない。Phase 2（発注）では iced modal で実行時取得・当該リクエスト完了後に即削除する方式を採用（env 経路・DTO 長期保持ともに使わない）
...
- **マスタキャッシュ保存先は Rust から Python へ明示的に受け渡す**。現行の `stdin` 初期 payload は `port` / `token` のみなので、T0 で `config_dir` / `cache_dir` を初期 payload に追加する。**現状実装**: [engine-client/src/process.rs](https://github.com/botterYosuke/e-station/blob/main/../engine-client/src/process.rs) の `build_stdin_payload` は `{port, token, dev_tachibana_login_allowed}` を送信済み。[python/engine/__main__.py](https://github.com/botterYosuke/e-station/blob/main/../python/engine/__main__.py) は `config_dir` / `cache_dir` を受け取れるが、Rust 側からはまだ送信していない（T4 着手待ち）。本節および architecture.md §2.1.1 / spec.md §3.1 は **T3/T4 完了後に成立する不変条件** であり、それまで Python 側 fast-path / マスタキャッシュ機能は実装されていない
