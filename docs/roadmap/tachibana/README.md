...
親計画は [docs/specs/data-engine/spec.md](../specs/data-engine/spec.md)（Rust ビュアー + Python データエンジン）。本計画はその上に「日本株（authenticated venue）」を載せるための差分仕様。
...
- **復旧シーケンスの source of truth は `ProcessManager`**。managed mode の再起動時は `Hello -> Ready -> SetProxy -> [Python 自律] startup_login -> VenueReady -> metadata fetch / resubscribe` を必ず再実行する（`SetVenueCredentials` は廃止済み。Python が自律的に `startup_login` を実行し `VenueReady` を送信する）。**T3.5 にて `VenueState` FSM / U4 Gate 着地済**: [src/venue_state.rs](../../../src/venue_state.rs) で FSM 化、[engine-client/src/process.rs](../../../engine-client/src/process.rs) の `apply_after_handshake` は SetProxy 後に `VenueReady` を非同期で待ち受け、受信後に resubscribe を発火する経路に更新済。U4 Gate により `VenueReady` 前の業務リクエストはブロックされる
