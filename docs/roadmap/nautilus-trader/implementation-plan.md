---
title: nautilus_trader 統合 実装計画
status: migrated
migrated_from:
  - docs/✅nautilus_trader/implementation-plan.md
source_commit: 8797909
---
...
- 更新ファイル: `src/replay_api.rs`（`ReplayApiState` 新設・3 エンドポイント追加・テスト 13 件追加）、`src/main.rs`（`is_replay_mode` を CLI mode から伝搬 + `ReplayApiState` を構築 + `replay_api::spawn` 引数追加）、`engine-client/src/connection.rs`（pre-existing dirty state を build できる最小 1 行修正 — 後述）、`docs/specs/backtest/implementation-plan.md`
- テスト結果:
...
- [x] ✅ **N1.11 完了 (2026-04-30, replay-market-data-emit.md)**: replay 用 market data の Rust UI 複製送出 — streaming ループ内で Bar → `KlineUpdate`、TradeTick → `Trades` を per-tick IPC emit。
...
- [ ] `docs/specs/venues/tachibana/implementation-plan.md` の Phase 2（発注）タスクが [docs/specs/order/](../specs/order/) と本計画 N2 に分離されていることを再確認し、引退表示を update（N2 着手時）
...
## レビュー反映 (2026-04-30, replay-market-data-emit review-fix-loop R1/R2)
