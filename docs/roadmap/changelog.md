---
title: "IPC スキーマ年表 (Changelog)"
status: migrated
migrated_from:
  - docs/✅python-data-engine/schemas/CHANGELOG.md
source_commit: bc6d8dd
---

> **SoT 注記**: SCHEMA_MAJOR/MINOR の真の Single Source of Truth は `engine-client/src/lib.rs` の履歴コメント。本ファイルは年表（要旨）として保持する。詳細な breaking 変更の根拠は ADR 0002 (IPC schema versioning) を参照。
