...
`scripts/run-replay-debug.sh` にストラテジファイルのパスを渡すだけです。
...
スクリプトが設定するデフォルトは以下のとおりです。上書きしたい場合は
環境変数かリポジトリ直下の `.env` ファイルで指定してください。
...
| `REPLAY_GRANULARITY` | `Daily` | 足種（`Daily` / `Minute` / `Trade`） |
| `REPLAY_INITIAL_CASH` | `1000000` | 初期資金（円） |

### 別の銘柄・期間で動かす

```bash
REPLAY_INSTRUMENT_ID=6758.TSE \
