...
**N-pre Q8 の決定次第で以下のいずれかを採用**:

- **案 A（暫定採用）**: `price_increment = Price(0.1, precision=1)` で固定し、`_compose_request_payload` が発注前に呼値丸めを行う。nautilus 内部の `price_increment` 超過 reject は `RiskEngine` の `max_order_price` / `min_order_price` で吸収
- 案 B: 価格帯ごとに `Instrument` を複数管理（実用上困難、非推奨）
- 案 C: 呼値テーブルの動的反映（`CLMIssueSizyouMstKabu` の `sYobine` フィールド）を N2 で前倒し実装

**Q8 解決後はこのセクションを確定版に書き換えること。**
...
**FOK 注意**: nautilus の `FOK`（一括約定 or 全取消）と立花の「不成（`sCondition=6`）」は意味が異なる（不成は「成行で発注し全部約定しなければ取消」に近いが仕様詳細は PDF 確認要）。N-pre Q8 解決後に正式写像を確定する。
