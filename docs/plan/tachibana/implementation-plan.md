...
- [ ] `TachibanaWorker.fetch_klines(timeframe="1d")` — `CLMMfdsGetMarketPriceHistory` 経由。IPC で受信する `timeframe` は **wire 形式 `"1d"`**（T0.2 L67 で `#[serde(rename = "1d")]` 確定済、Q36 / F-H1）。Rust 側 `Timeframe` enum 内部バリアント名は `D1` だが、Python 側は wire 文字列で受ける
...
- [ ] (B5 繰越) UI 統合 — engine-client 側 API は B4 で着地済 (`matches_tachibana_filter` / `ticker_meta_handle()` / `EngineConnection::capabilities()`、worktree branch tachibana/phase-1/T4-ui)。tickers_table::exchange_filter_btn 経路で実 call を実装するのは B5 タスク
  - **`reset_ticker_meta()` callsite は B5 で `src/main.rs` の `on_ready` クロージャから呼ぶ予定**。B4 (R1) では `EngineClientBackend::reset_ticker_meta()` の public API 公開のみで、production からの callsite はゼロ件のまま B5 に繰越。R3 レビューで「callsite 0 で reconnect 時の silent gap が閉じない」指摘 (H2) を受け、B5 着地時点で `on_ready` から呼出することを本計画書で明示する
...
- [ ] **B2: master 結合（銘柄→ yobine_code → tick の解決経路、data-mapping.md §5.4 と紐付け）**: B1 の純粋関数 lookup を `CLMIssueSizyouMstKabu` ロード経路に組み付ける:
...
- [ ] **Rust 側 `TickerInfo` 受信マッピング配線（HIGH-U-9）**: 現状 `engine-client/src/backend.rs::TickerMetadataMap` 構築は `TickerInfo::new(...)` 経路で `display_name_ja` / `lot_size` / `quote_currency` 正規化が落ちる。本タスクで以下を実装:
...
- [ ] **`tachibana.py::TachibanaWorker` クラスと `set_credentials(creds: TachibanaCredentials)` setter の新設（R11-3）**: 現状 `python/engine/exchanges/` に `TachibanaWorker` クラス自体が未実装（`tachibana_auth.py` のドキュメント文字列内で言及されるだけ）。T4 着手時に `tachibana.py` を新設して `TachibanaWorker` を定義し、`set_credentials` setter（is_demo 差分検知フックを内蔵）と `__init__`（`_master_loaded = asyncio.Event()` / `_master_lock = asyncio.Lock()` の初期化）を実装する
- [ ] **マスタ in-memory invalidation 規約（HIGH-U-10）**: `_master_loaded` Event とメモリ上のマスタ内容を再初期化（`clear()`）する経路を明文化。少なくとも以下 3 トリガで invalidate:
  - **`is_demo` フラグ変更時**: demo/prod 切替で別環境のマスタが残る穴を防ぐ。`TachibanaWorker.set_credentials(creds)` で `is_demo` の差分を検知し `_master_loaded.clear()` + メモリマップを破棄
  - **JST 日跨ぎ検知時（R11-2）**: `list_tickers` / `fetch_ticker_stats` 入口で **JST 日付（`YYYY.MM.DD` 形式）を比較**して前回ロード時の日付と異なれば invalidate。`tachibana_helpers.py::current_p_sd_date()` は `YYYY.MM.DD-hh:mm:ss.sss` のミリ秒精度タイムスタンプを返すため、**そのまま比較すると毎回不一致になる**。比較には先頭 10 文字（`YYYY.MM.DD`）を切り出す、または専用ヘルパ `current_jst_yyyymmdd() -> str` を `tachibana_helpers.py` に新設して使用する（後者推奨。`datetime.now(ZoneInfo("Asia/Tokyo")).strftime("%Y%m%d")` の薄ラッパで L531 のキャッシュファイル名生成と共通化できる）。キャッシュファイル名は L531 で日付別だが、長時間プロセスを跨ぐ場合のメモリ残存を防ぐ
  - **Python サブプロセス再起動時**: `TachibanaWorker.__init__` で必ず `_master_loaded = asyncio.Event()` を新規生成（プロセス自体が再起動するため自動だが規約として明記）
...
- [ ] **B5 follow-up: min_ticksize 解決**: `engine-client/src/tachibana_meta.rs::TACHIBANA_MIN_TICKSIZE_PLACEHOLDER_F32 = 1.0` 暫定値を `CLMYobine` テーブル経由 `yobine_code` lookup で正値解決 (B1 で実装済の `tick_size_for_price` Python 関数を IPC 経由で問い合わせ、または Rust 側で `YobineBand` cache を保持)
- [ ] **Phase 1 reconnect モデルの設計仮定（B4 R3 明記、H3）**: Phase 1 の reconnect モデルは `EngineClientBackend` 再構築前提とする。Python サブプロセス再起動時は `ProcessManager` が古い `EngineClientBackend` インスタンスを drop し、新しいインスタンスを構築する。`ticker_meta` (`Arc<Mutex<TickerMetaMap>>`) も古い backend と一緒に drop されるため、新インスタンスでは自動的にゼロ初期化された状態から開始される。**`EngineRehello` 由来の reset hook**（`update_handles` 経路で同じ backend インスタンスを使い回し、新たな Hello/Ready を受信して capabilities / ticker_meta を差し替えるケース）は **Phase 2 (T7) で追加**する。T7 で `EngineRehello` event/識別子を `dto.rs` / `process.rs` に追加した上で、backend 使い回し経路から `reset_ticker_meta()` を呼ぶ hook を実装する。本 Phase は新規構築モデルで silent gap が閉じることを設計仮定とし、B5 の `on_ready` callsite はこの仮定の下で `reset_ticker_meta()` を呼ぶ（新規構築でも防御的に呼ぶことで T7 hook 追加時の差分を最小化する）
