...
- [ ] **EC 重複検知 E2E**: 再接続を fault-injection で発生させ、同一 EC frame を 2 度受信しても `Event::OrderFilled` が 1 度しか発火しないこと（スクリプト未作成。単体テストは `test_ec_dedup.py` でカバー済み。Q11 の決定後に着手）
...
- [ ] **約定 toast 目視 E2E**: 発注 → 約定通知が UI に出ること（スクリプト未作成。Q11 の決定後に着手。GUI ありで目視確認）
