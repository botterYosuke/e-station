...
- **C-2: ForgetSecondPassword 競合ポリシー**（architecture.md §2.4 に仕様追記が先決）← **未着手**
- M-6: Python enum str パススルー（IPC 経由のため現実的リスク低い）← **LOW 残存**
- M-7: ClientOrderId pub → try_new() コンストラクタ対応 ← **LOW 残存**
...
- **C-2: ForgetSecondPassword 競合ポリシー** → architecture.md §2.4 仕様追記が先決 ← **未着手**
