...
- [ ] **第二暗証番号 IPC 漏洩防止テスト（D2-M5）**: `cargo test --test creds_no_second_password_on_wire` → Phase O0 で実施
...
- [ ] **Tpre.5 EC 仕様根拠の所在が確定** — Phase O0/O1 並行で実施。O2 着手前に必ず完了
...
- [x] ✅ **第二暗証番号 idle forget テスト（D2-M3）**: `python/tests/test_tachibana_session_holder.py` に idle forget / lockout / on_submit_success / on_invalid / 組み合わせテスト 17 件実装済み（ファイル名は計画と異なるが受け入れ条件をカバー）
...
- [ ] `client_order_id` 不明の注文は HTTP `/api/order/modify` `/api/order/cancel` 入力として **`venue_order_id` も受理**できるようにする（spec.md §5 に記載）
