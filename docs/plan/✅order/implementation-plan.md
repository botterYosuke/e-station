...
- **バグ（未修正）**: `_ACCOUNT_TYPE_MAP` の値・キー名が立花マニュアルと不一致（`"account_type=general"` が `"0"` → 存在しない値、`"account_type=specific_without_withholding"` が `"3"` → 実際は一般口座の値）。修正計画: `docs/plan/✅order/fix-account-type-map.md`
