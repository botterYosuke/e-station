...
- **将来の改善（未実装）**: EC イベント（`OrderFilled`/`OrderCanceled`/`OrderExpired`）受信時に自動で `GetOrderList` を発行する自動リフレッシュ機能。`map_engine_event_to_tachibana()` を `Option<Message>` → `Vec<Message>` に変更し `Message::OrderListNeedsRefresh` を追加する方針。詳細: `docs/plan/✅order/fix-order-list-auto-refresh-2026-04-28.md`
...
- [x] ✅ **B-M4 修正（2026-04-28）**: `_ACCOUNT_TYPE_MAP` をマニュアル確定値に修正。旧タグ名（`specific_with_withholding` 等）を廃止し `specific`/`general`/`nisa`/`nisa_growth` に統一。`src/api/order_api.rs` の旧タグ名も全置換済み（`docs/plan/✅order/fix-account-type-map.md`）
...
- [x] ✅ **後日バグ修正（2026-04-28）**: サイドバーから BuyingPower ペインを新規登録した場合、VenueReady 後でも `GetBuyingPower` が自動発行されなかった問題を修正。`OpenOrderPanel(ContentKind::BuyingPower)` ハンドラに `tachibana_state.is_ready()` + `buying_power_request_id.is_none()` ガード付きの auto-fetch を追加。VenueReady / BuyingPowerAction / OpenOrderPanel の 3 経路を `buying_power_request_id` 記録で対称化し、`EngineConnected` 時のリセットも追加。詳細: `docs/plan/✅order/fix-buying-power-auto-fetch-on-add-2026-04-28.md`。
...
- ~~[docs/plan/✅nautilus_trader/spec.md](../✅nautilus_trader/spec.md) §2.3 Phase N2 に「`tachibana_orders.py` を `LiveExecutionClient` 内で再利用」を明記~~ **N/A**: 変更不要。既に方針一致しており spec.md に追記しても冗長。N2 着手時に確認済みとみなす。
...
| C-H3 | `(venue_order_id, trade_id)` 重複検知テストは `test_ec_dedup.py`（7 テスト）に実装済みを確認。`invariant-tests.md` を ✅ に更新 | `docs/plan/✅order/invariant-tests.md` | ✅ |
