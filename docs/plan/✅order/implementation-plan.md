...
- [ ] **発注 E2E** `s80_order_submit_demo.sh`（スクリプト未作成）: `curl POST /api/order/submit` で「現物・成行・買 100 株」が通り `sOrderNumber` が返ること
  - **前提条件**: `.env` に `DEV_TACHIBANA_USER_ID` / `DEV_TACHIBANA_PASSWORD` / `DEV_TACHIBANA_DEMO=true` が揃っており実行可能（2026-04-28 確認済み）
  - **ブロッカー**: 第二暗証番号のヘッドレス注入方法が未決定（[open-questions.md Q11](./open-questions.md) 参照）。解決後にスクリプトを新規作成する
  - ログイン E2E は `tests/e2e/tachibana_demo_login.sh` として既存・実行可能
- [ ] **クラッシュリカバリ E2E** `s80_order_crash_recovery_demo.sh`（スクリプト未作成）:
...
- [ ] **訂正・取消 E2E** `s81_order_modify_cancel_demo.sh`（スクリプト未作成）: デモ環境で「指値発注 → 訂正 → 取消」が curl から完結すること。**ブロッカー**: Q11 の決定待ち
