...
## 実装トラック詳細（tachibana → order → nautilus_trader）

本ロードマップの Phase 1〜4 に加え、**3 つの実装計画トラック**が逐次依存する形で並走する。

```
[tachibana Phase 1]  ──T2完了──▶  [order Phase O-pre〜O3]  ──O0〜O2完了──▶  [nautilus Phase N0〜N3]
 閲覧基盤                           発注経路                                   エンジン置換
 IPC schema 1.1 → 1.2             IPC schema 1.2 → 1.3                       IPC schema 1.3 → 1.4
```

各計画の詳細: [tachibana/](tachibana/) | [order/](order/) | [nautilus_trader/](nautilus_trader/)

### 着手ブロッカー一覧

| 計画 | 着手条件 | 完了時に解除されるブロッカー |
|---|---|---|
| **tachibana T0〜T7** | python-data-engine 完了済み | order/ Phase O-pre 着手可能（tachibana T2 完了が条件） |
| **order O-pre〜O3** | tachibana T2（認証実装）完了 | nautilus N1 着手条件の一部（O0 完了）、nautilus N2 着手条件の一部（O0〜O2 完了） |
| **nautilus N-pre〜N0** | python-data-engine 完了済み（order/ と並行着手可） | — |
| **nautilus N1** | N0 完了 + order/ O0 完了 | — |
| **nautilus N2** | N1 完了 + tachibana Phase 1 完了 + order/ O0〜O2 完了 | — |
| **nautilus N3** | N2 完了 | Rust `exchange/` 発注コード撤去（N-pre Tpre.4 の grep で存在が確認できた場合のみ） |

### IPC スキーマバージョン連鎖

各トラックは IPC スキーマバージョンに順次依存する。後続トラック着手前に前バージョンの**ラウンドトリップテストが緑**であることが必須条件。

| バージョン | 担当計画・タスク | 主な追加内容 |
|---|---|---|
| **1.1 → 1.2** | tachibana T0.2 | `SetVenueCredentials` / `VenueReady` / `VenueError` / `TachibanaCredentialsWire` / `TachibanaSessionWire` |
| **1.2 → 1.3** | order Tpre.2 | `SubmitOrderRequest` / `OrderSide` / `OrderType` / `TimeInForce` / `OrderModifyChange` / `OrderEvent::*` / `SetSecondPassword` / `ForgetSecondPassword` / `SecondPasswordRequired` |
| **1.3 → 1.4** | nautilus N1.1 | `StartEngine` / `StopEngine` / `EngineStarted` / `EngineStopped` / `PositionOpened` / `PositionClosed` / `AdvanceClock`（N-pre Tpre.1 の結論次第） |

### 計画間の主要な再利用・置換関係

| 上流の成果物 | 下流での扱い |
|---|---|
| tachibana `tachibana_auth.py` / `tachibana_url.py` / `tachibana_codec.py` | order/ が全面再利用（認証・session・URL ビルダ・codec） |
| order/ `tachibana_orders.py`（`submit_order` / `modify_order` / `cancel_order`） | nautilus N2 で `LiveExecutionClient.submit_order` の委譲先として再利用。**関数シグネチャは変更しない**（nautilus 互換不変条件）|
| order/ HTTP API `/api/order/*` | nautilus N1/N2 完了後も**残存**（手動発注・curl 経路として維持）|
| docs/plan/README.md Phase 2「自作 Virtual Exchange Engine」 | nautilus N1 で `BacktestEngine` に置換・**破棄**。N1.2 着手前に `git grep` で現リポジトリに自作実装がないことを確認してから削除判断 |
| docs/plan/README.md Phase 4a ナラティブ API | nautilus N1.5 で `Strategy.on_event` フックから既存 `/api/agent/narrative` への配線を追加。既存 API は変更しない |
