...
| 同 `submit_new_order` / `submit_cancel_order` 関数 | `python/engine/exchanges/tachibana_orders.py` の `submit_new_order` / `submit_modify_order` / `submit_cancel_order` / `submit_cancel_all` として移植（**nautilus 用語の `modify` で統一**。立花 "correct" 用語は `_compose_request_payload` 内部に閉じる） |
| 同 `serialize_order_request()`（`p_no` / `p_sd_date` / `sCLMID` / `sJsonOfmt` / 逆指値デフォルトを後付け） | Python 側 `tachibana_orders._compose_request_payload()` として再現。`tachibana_auth.next_p_no()` / `current_p_sd_date()` を使う |
...
| [docs/plan/✅python-data-engine/](../✅python-data-engine/) | IPC スキーマ 1.2 → 1.3 へ bump。`Command::SubmitOrder` / `CancelOrder` / `CorrectOrder` および `Event::OrderAccepted` / `OrderFilled` / `OrderRejected` を追加 |
