...
> Status: この文書は旧 `iced` カスタムウィジェット案の実装メモです。
...
各 `FloatingPane` はキャンバス座標系での位置・サイズ（`FloatRect`）と z_index を持ち、
z 順は各 `FloatingPane` の `z_index: u32` フィールドで管理する（値が大きいほど最前面）。
...
> **Camera の pan 型について**: `data::Camera`（永続化層）は `pan: (f32, f32)` を使用する
> （`iced::Point` は非 Serialize なため）。ウィジェット内の Camera は `pan: iced::Point` を
> 使用する。変換時は `(pan_x, pan_y)` ↔ `iced::Point { x, y }` を相互変換する。
...
| `CursorMoved` | `drag` 中 | delta をワールド座標に逆変換して `InternalState` の drag_rect を更新（`on_move` は発行しない） |
...
| `MouseButtonReleased(Left/Middle)` | `drag` 中 | 最終位置を確定して `on_move` 発行、`drag` 解除 |
| `MouseButtonReleased(Left/Middle)` | `drag` 以外 | `resize` / `pan` 解除 |
...
pub struct Dashboard {
    ...
    pub popout:  Vec<(Vec<FloatingPaneData>, WindowSpec, Camera)>,  // popout ごとに独立した Camera を保存
    #[serde(default)]
...
pub struct FloatingPane {
    pub id:      uuid::Uuid,
    pub rect:    FloatRect,
    pub z_index: u32,        // z 順管理（値が大きいほど最前面）
...
### `pane::State::view()` の戻り値型変更（`src/screen/dashboard/pane.rs` の `view` 関数）
...
```rust
// 変更前
pub fn link_group_button<'a, Message: 'a, F>(
    id: iced::widget::pane_grid::Pane,
    on_link: F,
) -> Element<'a, Message>
where
    F: Fn(iced::widget::pane_grid::Pane) -> Message,
```

```rust
// 変更前（widget.rs:238）
// link_group_button 関数の引数 id: pane_grid::Pane と
// F: Fn(pane_grid::Pane) -> Message クロージャ境界を uuid::Uuid に変更する
pub fn link_group_button<F>(
    id: pane_grid::Pane,   // ← uuid::Uuid に変更
    on_change: F,
) -> Element<Message>
where
    F: Fn(pane_grid::Pane) -> Message,  // ← Fn(uuid::Uuid) -> Message に変更
```

LinkGroup 機能は継続するが、`link_group_button` 関数の引数 `id: pane_grid::Pane` と
`F: Fn(pane_grid::Pane) -> Message` クロージャ境界を `uuid::Uuid` に変更する。
Phase 2 で実施。

### `src/main.rs` での `focus` フィールド直接操作（6箇所）
...
| main.rs:2988–2990 | Layout メニュー表示時の `focus` 参照（メニュー項目の有効/無効制御） |
    ...
    fn from(dashboard: &Dashboard) -> Self {
        data::Dashboard {
            ...
            popout: dashboard.popout.iter()
                .map(|(_, (panes, spec, cam))| {
                    (panes.iter().map(floating_pane_to_data).collect(), *spec, *cam)
                })
                .collect(),
...
| 項目 | 変更前 | 変更後 |
|------|--------|--------|
| 内部状態 | `pane_grid::State<pane::State>` | `Vec<FloatingPane>` + `Camera`（独立） |
| 表示 | `PaneGrid::new(state, ...)` | `FloatingPanes::new(&state, ...)` |
| 永続化型 | `(Vec<FloatingPaneData>, WindowSpec)` | `(Vec<FloatingPaneData>, WindowSpec, Camera)` |
| 意味 | popout ウィンドウ内でさらにスプリット可能 | popout ウィンドウ内でもフローティング、Camera はメイン非依存 |
...
**Camera 座標変換ユニットテスト（`data/src/layout/mod.rs` に追加）:**

```rust
#[test]
fn camera_roundtrip() {
    let camera = Camera { pan: (100.0, 50.0), zoom: 2.0 };
    // data::Camera の (f32,f32) pan をウィジェット Camera の iced::Point に変換して検証
    let p = iced::Point { x: 42.0, y: 17.0 };
    let screen = camera.world_to_screen(p);
...
コマンド: `cargo test -p data camera_roundtrip`
...
- `src/main.rs` — `dashboard.focus` の直接参照・更新（6箇所）を `uuid::Uuid` ベースに修正
...
zoom 値変化時のフレームレート実測: `cargo run` (debug ビルド) でズーム操作中の FPS 低下が 20% 以内であることを目視確認する。低下が大きい場合は zoom の変化率を `ZOOM_STEP` 以下に絞るか、`shell.request_redraw()` の呼び出し頻度を調整する。
...
ファイル: `data/src/layout/dashboard.rs`
コマンド: `cargo test -p data legacy_pane_format_falls_back_to_empty`

```rust
#[test]
fn legacy_pane_format_falls_back_to_empty() {
    // 旧フォーマット（pane キーを持つが windows キーがない JSON）
    let old_json = r#"{"pane":{"kind":"Starter"}}"#;
    let result: Dashboard = serde_json::from_str(old_json).unwrap();
    assert!(result.windows.is_empty(),  "windows は空 Vec にフォールバックする");
    assert_eq!(result.camera.zoom, 1.0, "camera はデフォルト値にフォールバックする");
    assert_eq!(result.camera.pan,  (0.0, 0.0));
}
```

#### Vec 順序固定の不変条件 pin テスト

ファイル: `src/widget/floating_panes.rs`
コマンド: `cargo test -p flowsurface vec_order_stable_on_focus_change`

```rust
#[test]
fn vec_order_stable_on_focus_change() {
    // フォーカス変更後も windows[i].id の順序が変わらないことを保証する
    // （z_index 変更のみで並べ替えは行わない）
    let ids_before: Vec<_> = windows.iter().map(|w| w.id).collect();
    focus_window(&mut windows, target_id);
    let ids_after: Vec<_> = windows.iter().map(|w| w.id).collect();
    assert_eq!(ids_before, ids_after, "Vec の順序はフォーカス変更で変化しない");
}
```

#### popout Camera 独立性テスト

ファイル: `src/screen/dashboard.rs`
コマンド: `cargo test -p flowsurface popout_camera_independent`

```rust
#[test]
fn popout_camera_independent() {
    // メインウィンドウの CameraChanged が popout の Camera に影響しないことを保証
    let popout_cam_before = dashboard.popout_camera(popout_window_id);
    dashboard.update(Message::CameraChanged(new_camera));
    let popout_cam_after = dashboard.popout_camera(popout_window_id);
    assert_eq!(popout_cam_before, popout_cam_after);
}
```

#### Negative テスト（存在しない UUID の WindowClosed でパニックしない）

ファイル: `src/screen/dashboard.rs`（または `src/widget/floating_panes.rs`）
コマンド: `cargo test -p flowsurface close_nonexistent_pane_is_noop`

```rust
#[test]
fn close_nonexistent_pane_is_noop() {
    let nonexistent = uuid::Uuid::new_v4();
    // パニックせず、windows の長さが変わらないことを保証
    let len_before = dashboard.windows.len();
    dashboard.close_pane(nonexistent);
    assert_eq!(dashboard.windows.len(), len_before);
}
```

#### z_index オーバーフロー対策

`z_index: u32` が `u32::MAX` に近づいた場合、フォーカス時に再正規化する:
全 `FloatingPane` の `z_index` を連番（0, 1, 2, …）に振り直し、オーバーフローを防ぐ。
再正規化は `z_index >= u32::MAX - windows.len() as u32` を条件にトリガーする。

#### E2E smoke テスト更新

ファイル: `tests/e2e/smoke.sh`

以下の観測項目を追加する:
- `WindowMoved` ログが出力されること（FloatingPane のドラッグが機能していること）
- `floating_panes` / `FloatingPane` に関するクラッシュログが観測ウィンドウ中に出ないこと
