---
title: "data-engine 仕様: Rust ビュアー + Python データエンジン"
status: migrated
migrated_from:
  - docs/✅python-data-engine/spec.md
...
### 3.1 起動フロー（外部エンジン自動 attach + spawn フォールバック）

`--data-engine-url` を指定しない場合、Rust 起動時は以下の順序で動作する：

1. **環境変数チェック**（`FLOWSURFACE_ENGINE_TOKEN`）
   - 未設定または空 → 手順 3（spawn）へ直行（無駄な遅延を避けるショートサーキット）
   - 設定あり → 手順 2 へ
...
> **ロックファイル案は不採用。** 当初 `engine-discovery.md` で検討したが、
> 固定ポート 19876 プローブ方式（HMAC + SCHEMA_MAJOR 二重検証で必ず弾ける）に変更した。

`--data-engine-url` 指定時は従来通り外部接続固定（失敗はエラー終了）。

### 3.2 異常終了・再起動

- 異常終了時は Rust が再起動を試行（指数バックオフ、最大 N 回）。
- 開発時に Python を独立起動する場合、`--data-engine-url` フラグで明示的に指定することも可能（既存のユースケースを維持）。
...
正本は `python/engine/schemas.py` の pydantic モデル群と `engine-client/src/dto.rs` の Rust DTO（共に `SCHEMA_MAJOR=3 / SCHEMA_MINOR=9`、2026-05-04 時点。Phase 8.1b で `ClientConnected` / `ClientDisconnected` / `EngineBusy` を追加、R1〜R4 レビュー反映で `EngineBusy.request_id` / `EngineBusy.current_state` の `ReplayStateName | LiveStateName` 直交 union model_validator / `AUTH_FAILED_CODE` 共有定数 / `AttemptedCommand` Literal を追加）。表は分類目的で、追加・削除があれば schemas.py の `class \w+\(IpcMessage\)` 列挙と本表の差分を [`roadmap/changelog.md`](../roadmap/changelog.md) と一緒に更新する。

※ `exchange::Event` 列挙体の各バリアント（`Connected` / `Disconnected` / `DepthReceived` / `TradesReceived` / `KlineReceived`）と一対一対応する形で Python→Rust イベントを定義する。OI インジケータが継続的に要求する `FetchRange::OpenInterest` も `FetchOpenInterest` コマンド + `OpenInterest` レスポンスで表現する（参考: `exchange/src/adapter/client.rs` `fetch_open_interest`、`src/chart/indicator/kline/open_interest.rs`）。

ticker 一覧 / metadata / stats は現行 `AdapterHandles` が起動直後に取得している（`exchange/src/adapter/client.rs` L200 付近・L269 付近）。新構成でも `ListTickers` / `GetTickerMetadata` / `FetchTickerStats` を trait と IPC の両方に明示し、`EngineClientBackend` が同じ API を提供することで UI 側の起動シーケンスを不変に保つ。
...
- `Trade` と `TickerStats` のみ `Deserialize` 派生を持つが、`Kline`（`exchange/src/lib.rs` L544〜）・`OpenInterest`（同 L651〜）は serde 派生なし。
- `Depth`（`exchange/src/depth.rs`）は内部表現寄りで、そのままシリアライズすると Python 側が読みにくい。
- `exchange::Event`（`exchange/src/adapter.rs` L535〜）は `Arc<Depth>` / `Box<[Trade]>` を含み、IPC で直接扱う shape ではない。
...
- 共通スキーマ定義は [`reference/schemas/`](../reference/schemas/) 配下に JSON Schema として配置し、Rust `serde` 派生 / Python `pydantic` モデルの両方を同スキーマから生成する（生成器の選定は [open-questions](../roadmap/data-engine/open-questions.md) 参照）。
...
1. **Rust → Python: `Hello`**
   ...
   - **`mode` (N1.13 追加)**: 起動時に固定する動作モード。replay モードの取り扱いは [`specs/replay.md`](./replay.md) を参照。旧クライアント互換のため省略時は `"live"` にフォールバック。
...
4. **Python: venue startup login（autonomous、schema 2.x 以降）**
   - `Hello`/`Ready` 完了後、Python は `tachibana_account.json` / `tachibana_session.json` を起点に **自律的に** Tachibana の startup login を進める。Rust から `SetVenueCredentials` を送って起動を gate する旧モデルは廃止された（`engine-client/src/process.rs` の "schema 2.x — autonomous login (no SetVenueCredentials → VenueReady gate)" コメントが正本記述）。
   - **`mode == "replay"` のときの扱い**は [`specs/replay.md`](./replay.md) を参照。
   ...
   - ユーザーが UI から再ログインを要求した場合のみ Rust は `RequestVenueLogin` を発火する（Python 側内部の自動再ログインは禁止、tachibana spec §3.2）。
...
- [`roadmap/changelog.md`](../roadmap/changelog.md) に major/minor 変更履歴を記録する。
...
現状の `AdapterHandles` は venue 毎の具体ハンドルを直接フィールドに持ち、`spawn_all()` で一斉起動している（`exchange/src/adapter/client.rs` L21〜、L30〜）。
...
- trait `VenueBackend` を定義。現行 `AdapterHandles` が担う全経路を覆う:
  - 初期化系: `list_tickers` / `get_ticker_metadata`（`exchange/src/adapter/client.rs` L200 付近・L269 付近に対応）
...
この抽象化はフェーズ 1 の前提となるため、[implementation-plan](../roadmap/data-engine/implementation-plan.md) のフェーズ 0.5 として切り出す。
...
現行ではプロキシ認証は OS keyring から復元され（`src/layout.rs` 付近）、起動時に `AdapterHandles::spawn_all()` へ渡されている（`src/main.rs` 付近）。Python サブプロセス化に伴い以下を決める:
...
python/
├── engine/
│   ├── __main__.py          # エントリポイント: `python -m engine`（ポートは stdin JSON ペイロードで受け取る）
│   ├── server.py            # WS サーバ・dispatch
│   ├── schemas.py           # pydantic モデル（SCHEMA_MAJOR/MINOR 定義）
...
- 主要依存: `httpx`（REST）、`websockets`（WS クライアント＆サーバ）、`pydantic`、`uvloop`（Linux/macOS のみ。Windows は対象外 → §6.2 参照）、`orjson`。
- 配布: 開発時は `uv` / `pip` セットアップ、リリース時は `PyInstaller` で同梱バイナリ化（決定は [open-questions](../roadmap/data-engine/open-questions.md) 参照）。
...
**判断基準**: フェーズ 2 完了時の IPC レイテンシ計測結果と、実ユーザーからの要望。現時点の暫定方針は **案 A**（撤去）。フェーズ 2 終了時点で再判断する（[open-questions Q5](../roadmap/data-engine/open-questions.md) を clos 条件に変更）。
...
- **フェーズ 0 で現行 Rust 直結のベースラインを取得**し、[`testing/benchmarks/`](../testing/benchmarks/) に記録。以降のフェーズで同条件で再測して比較する。
- 計測項目:
