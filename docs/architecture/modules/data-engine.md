---
title: "data-engine モジュール アーキテクチャ"
status: migrated
migrated_from:
  - docs/✅python-data-engine/current-architecture.md
...
> - [implementation-plan](../../roadmap/data-engine/implementation-plan.md) — Phase 0〜8 の完了状態
> - `python-helper-direct-api.md`（旧 archive。ADR 化候補） — Phase 8 実装詳細（✅ 完了）
> - [data-engine 仕様](../../specs/data-engine.md) — 現行 IPC 仕様
>
...
- **Open Interest**: `src/chart/indicator/kline/open_interest.rs` がインジケータとして継続的に `FetchRange::OpenInterest` を要求。→ MVP 必須。
- **Ticker stats (24h)**: `TickerStats`（`exchange/src/lib.rs` L640 付近）。現行 `tickers_table.rs` で 24h 変化率・出来高・`mark_price` を表示するため取得。→ MVP 必須（`mark_price` はここに含まれる）。
- **Kline 履歴フェッチ**: `src/connector/fetcher.rs` 経由、ユーザー操作時（スクロール・期間変更）。
...
1. 起動時 `AdapterHandles::spawn_all()` で全取引所の `NativeBackend` を spawn (`exchange/src/adapter/client.rs`)。
...
5. `src/screen/dashboard.rs` の `ingest_depth` / `ingest_trades` / `update_latest_klines` が消費（変更なし）。
6. インメモリ構造体に保持し、Iced が毎フレーム再描画。永続化 DB は無し。

**追加された型・ファイル**:
- `exchange/src/adapter/venue_backend.rs`: `VenueBackend` trait、`NativeBackend` enum、`TickerMetadataMap` / `TickerStatsMap` 型エイリアス。
- `exchange/tests/venue_backend.rs`: trait 抽象化の統合テスト。
