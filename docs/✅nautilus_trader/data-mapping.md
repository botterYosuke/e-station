...
通常運用の reason_code 体系（`MARKET_CLOSED` / `INSUFFICIENT_FUNDS` 等）は [`docs/✅order/spec.md §5.2`](../✅order/spec.md#52-reason_code-体系観測性) を参照する。本表は N1 期間中に追加された REPLAY 専用の暫定 code のみを記載する。
