...
ラウンド 11 で MEDIUM ゼロ収束を再達成。LOW（invariant-tests.md SKILL R7 の誤紐付け）はラウンド 11 中に R11-1 の修正と一括で解消済み。本ラウンドをもって `docs/✅tachibana/` の Phase 1 計画レビューループは完全収束し、T4 着手の全前提（wire 形式・正規化責務・ゲート対象・検索フィールド名・`TachibanaWorker` 新設・JST 日付ヘルパ・invariant 表登録）が揃った。

## ラウンド 12（2026-04-26 B1 調査結果反映 / `CLMYobine` per-stock 解決への前提書換）

B1 調査で判明した事実（`CLMYobine` が呼値テーブルの正本、`sYobineTaniNumber` ごとに 20 段 band、`CLMIssueSizyouMstKabu.sYobineTaniNumber` で銘柄から参照、PDF §2-12 は構造説明であり全価格帯テーブルの完全定義ではない）を計画書に反映。旧前提（PDF §2-12 を単一テーブルで hardcode + `tick_size_for_price(price: Decimal) -> Decimal` 単一引数）を撤回。

| 重要度 | Finding | 対応ファイル | 変更内容 |
| :--- | :--- | :--- | :--- |
| HIGH B1-1 | `data-mapping.md §5` の冒頭が「価格帯ごとに刻みが変わる」+ 単一価格帯テーブル例示 + 旧 (A)(B)(C) 案を提示し、`tick_size_for_price(price: Decimal) -> Decimal` を 1 関数 hardcode する旧前提が L100-115 に残存。L117 は新前提で書換済だが矛盾状態 | `data-mapping.md §5` | 冒頭を「呼値は銘柄ごとに `CLMYobine` 行を引いて求める」「`api_request_if_master_v4r5.pdf §2-12` は構造説明、全銘柄共通の単一表は存在しない」「実値は `CLMYobine` master download で常に取得」に書換。例示ブロックは「**hardcode 用ではなくテーブル形式の理解用**」と明記。(A) を「per-stock 1 値固定（Phase 1 採用）」に再定義し、`tick_size_for_price(price, yobine_code, yobine_table)` 戻り値を `Decimal -> f32` で `TickerInfo::new_stock` に詰める手順に書換 |
| HIGH B1-2 | `MASTER_CLMIDS` リスト（implementation-plan.md L533）に `CLMYobine` が無く、master download 経路の型ガードから漏れる | `implementation-plan.md` MEDIUM-C7 | `MASTER_CLMIDS` 例示に `CLMYobine` を追加、「呼値テーブル正本、data-mapping.md §5 / B1 改訂と整合」と明記 |
| HIGH B1-3 | `tick_size_for_price` 境界値テスト記述（HIGH-D2-1）が「PDF §2-12 全価格帯境界」を悉皆する旧前提のまま、新 signature に未追従 | `implementation-plan.md` T4 HIGH-D2-1 | 旧記述を撤回し「B1: `CLMYobine` decoder + `tick_size_for_price` lookup」として新 signature と既存 TDD ファイル（`python/tests/test_tachibana_yobine.py`）の 5 テスト pin に書換。代表 yobine_code（`101`/`103`/`418`）の境界値±1 銭で足りる旨明記。B1 完了マーク `[x]` |
| HIGH B1-4 | 銘柄→ yobine_code → tick の master 結合経路（B2 相当）が T4 タスクに不在。B1 純粋関数だけでは `TickerInfo::new_stock` に詰める経路が完成しない | `implementation-plan.md` T4 | `[ ]` B2 master 結合タスクを新規追加: `resolve_min_ticksize_for_issue(issue_record, yobine_table, snapshot_price)` 実装、`_ensure_master_loaded()` での `CLMYobine` download 並行化、`yobine_table` invalidation を HIGH-U-10 と同一ライフサイクル（is_demo / JST / `__init__`）で揃える。テスト pin: `test_resolve_tick_size_for_issue_uses_clm_yobine_lookup`（3 ケース）+ `test_yobine_table_reloaded_on_invalidation_triggers`（3 トリガ parametrize） |
| MEDIUM B1-5 | `spec.md §2.1` の「銘柄マスタから合成」が `CLMIssueMstKabu` + `CLMIssueSizyouMstKabu` のみ列挙、`CLMYobine` が抜けて呼値解決経路が読めない | `spec.md §2.1` | 合成ソースに `CLMYobine` を追加、「呼値は per-stock 解決（`sYobineTaniNumber` で `CLMYobine` 行を引いて band を選ぶ）であり、全銘柄共通の単一価格帯テーブルは存在しない」を併記 |
| MEDIUM B1-6 | `spec.md §2.1` の「立花特有の呼値単位を反映」が実装源泉未明記 | `spec.md §2.1` | 「実装源は `CLMYobine` + `CLMIssueSizyouMstKabu.sYobineTaniNumber`、Phase 1 でも銘柄別呼値を使用」を併記 |
| MEDIUM B1-7 | `open-questions.md Q2` が「(A) 最小値固定 = 単一テーブル」前提のまま。資料_呼値の所在不明問題が open のように読める | `open-questions.md Q2` | 決定欄を「(A) per-stock 1 値固定」に再定義、`CLMYobine` 由来 band lookup の方針と、資料_呼値問題が `CLMYobine` 実データ取得で Phase 1 実装前提として解決済みである旨を明記 |
| MEDIUM B1-8 | B1/B2 関連 7 test 関数が `invariant-tests.md` 表に未登録。CI grep ガード導入時に未対応 ID 警告が出る | `invariant-tests.md` 表 | HIGH-D2-1-B1a〜B1e（B1: 5 test）+ HIGH-D2-1-B2a/B2b（B2: 2 test）の 7 行を表末尾に追加 |

ラウンド 12 で B1 調査事実が plan / spec / data-mapping / open-questions / invariant-tests に整合反映され、B1 は `[x]` 完了、B2 は `[ ]` 着手可能状態に整った。`tick_size_for_price(price)` 単一引数 / `101` 固定 / 単一 hardcode 表 / 「資料_呼値が外部別資料」の旧前提はすべて撤回。
