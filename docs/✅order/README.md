...
| [docs/✅python-data-engine/](../✅python-data-engine/) | IPC スキーマ 1.2 → 1.3 へ bump。`Command::SubmitOrder` / `ModifyOrder` / `CancelOrder` および `Event::OrderAccepted` / `OrderFilled` / `OrderCanceled` / `OrderRejected` を追加（nautilus 用語に統一。立花 "correct" は IPC 層に出さない） |
