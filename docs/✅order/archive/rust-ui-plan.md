...
詳細設計: `docs/✅order/fix-order-list-auto-refresh-2026-04-28.md`（未実装）。
...
**後日バグ修正（2026-04-28）**: サイドバーから BuyingPower ペインを新規登録した場合、VenueReady 後でも `GetBuyingPower` が自動発行されなかった（VenueReady ハンドラは起動時 1 度しか走らないため）。`OpenOrderPanel(ContentKind::BuyingPower)` ハンドラに auto-fetch ロジックを追加して修正。VenueReady / BuyingPowerAction / OpenOrderPanel の 3 経路を `buying_power_request_id` 記録で対称化。詳細: `docs/✅order/fix-buying-power-auto-fetch-on-add-2026-04-28.md`。
