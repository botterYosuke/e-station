...
def test_subscribe_kline_accepts_optional_timeframe():
    s = Subscribe(venue="binance", ticker="BTCUSDT", stream="kline", timeframe="1m")
    assert s.timeframe == "1m"


def test_open_interest_message_shape():
    msg = OpenInterestMsg(
        request_id="req-1",
        venue="binance",
        ticker="BTCUSDT",
        data=[{"ts_ms": 1700000000000, "open_interest": "12345.67"}],
    )
    assert msg.data[0].ts_ms == 1700000000000
