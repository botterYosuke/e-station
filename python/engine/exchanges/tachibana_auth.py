...
* `validate_session_on_startup(session, *, _latch)` — light-weight
  ...
  path instead — see [architecture.md §6](../../../docs/specs/venues/tachibana/architecture.md#6-失敗モードと-ui-表現).
...
# Tachibana p_errno codes that indicate "server is currently outside service
# hours" rather than a credential problem. Observed values:
#   "-62" — システムサービス時間外 (CLMAuthLoginRequest before/after demo hours)
# Keep this list narrow: only codes whose server message is unambiguously
# "out of service hours" belong here. The banner stays under
# `code="login_failed"` (so Rust severity / re-login button is unchanged);
# only the message differs (architecture.md §6, F-Banner1 explicitly allows
# Python-side message differentiation without Rust changes).
...
def _raise_for_error(data: dict[str, Any], *, login_path: bool) -> None:
    ...
    if login_path:
        ...
        if err.code in _SERVICE_OUT_OF_HOURS_CODES:
            raise LoginError(code=err.code, message=_MSG_SERVICE_OUT_OF_HOURS)
