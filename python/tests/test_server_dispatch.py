...
from flowsurface_data.schemas import SCHEMA_MAJOR, SCHEMA_MINOR
...
async def running_server(unused_tcp_port):
    ...
    with patch(
        "flowsurface_data.server.BinanceWorker",
...
def _make_mock_worker():
    ...
    worker.subscribe_trades = MagicMock(return_value=asyncio.Queue())
    worker.subscribe_depth = MagicMock(return_value=asyncio.Queue())
    worker.subscribe_kline = MagicMock(return_value=asyncio.Queue())
    worker.unsubscribe = AsyncMock()
    return worker
