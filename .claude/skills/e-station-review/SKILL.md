...
- pending intent / deferred replay の clear 条件
...
## 追加チェック: pending replay の取り消し忘れ

gate で「今は実行せず、あとで replay する」設計は、成功条件だけでなく取消条件まで追わないと危険。
- `pending=true` を立てた箇所ごとに、`OFF` / dismiss / cancel / unselect / retry failure / reconnect reset でどう落ちるか確認する
- replay 実行側が `pending` だけを見ていないか確認する。実行時点の `selected` / `enabled` / `state` も再確認しているかを見る
- 「一度選んでから解除したのに、後で ready が来ると replay する」型のズレを必ず疑う
- `new()` / `restore` / `update_handles()` / reconnect path でも pending が意図せず温存されていないか確認する
- pending を立てる positive test だけでなく、「途中で取り消したら replay しない」negative test を要求する

特に次を疑う。
- `set_*_ready(true)` や replay helper が `pending` だけで fetch / subscribe を再送する
- UI の selected state は外れているのに、内部 pending だけが残る
- reset 時に pending を残す判断が、unselect まで巻き込んでいない
...
## 追加チェック: UI 導線が本当に成立しているか

state machine や error class が正しくても、実際の UI 導線が message 形式に依存していると簡単に死ぬ。
- action button の表示条件を最後まで追う。`class.action()` だけで出るのか、message の行数や label 同梱が別途必要なのか確認する
- Python/Rust 分担がある場合、片側が「文言だけ送る」、もう片側が「3 行目がある時だけボタンを出す」ような暗黙契約を疑う
- banner / toast / dialog / inline button のうち、どれが唯一の復旧導線かを特定し、それが実データで本当に表示されるか確認する
- fixed message 定数を grep して、UI renderer の parse 条件と噛み合っているかを確認する
- 「分類テーブルの test はあるが、ユーザーが押せるボタンの test がない」場合は不十分として扱う

特に次を疑う。
- `Relogin` / `Dismiss` に分類されているのに、message 形状不足でボタンが出ない
- single-line message を想定している emitter と multi-line parse を前提にする renderer がずれている
- banner が no-op でも toast があるから大丈夫、と誤認している
