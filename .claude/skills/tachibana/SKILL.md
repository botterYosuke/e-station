...
> - **env 名は venue prefix 付き**: `DEV_TACHIBANA_USER_ID` / `DEV_TACHIBANA_PASSWORD` / `DEV_TACHIBANA_SECOND_PASSWORD` / `DEV_TACHIBANA_DEMO`。**読むのは Python 側 `tachibana_login_flow`**（Rust 側に `#[cfg(debug_assertions)]` の env 取込みは不要）
...
`DEV_TACHIBANA_USER_ID` / `DEV_TACHIBANA_PASSWORD` / `DEV_TACHIBANA_SECOND_PASSWORD` / `DEV_TACHIBANA_DEMO` による自動ログインは **Python 側 `python/engine/exchanges/tachibana_login_flow.py`** が読む。Rust 側に `#[cfg(debug_assertions)]` の env 取込みコードは追加しない（経路が Python に閉じる）。release ビルドでは `DEV_TACHIBANA_*` を読み込まず常にユーザー入力を要求する。
