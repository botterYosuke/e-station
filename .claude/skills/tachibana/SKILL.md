...
> ⚠️ **同期中（2026-04-25 時点）**: 本スキルは `exchange/src/adapter/tachibana.rs` / `data/src/config/tachibana.rs` / `src/screen/login.rs` / `src/connector/auth.rs` / `src/replay_api.rs` などを実在する参考実装として記述しているが、**現リポジトリには存在しない**（計画フェーズ）。また env 変数名 `DEV_USER_ID` 系は旧表記で、**新計画では `DEV_TACHIBANA_USER_ID` / `DEV_TACHIBANA_PASSWORD` / `DEV_TACHIBANA_SECOND_PASSWORD` / `DEV_TACHIBANA_DEMO` に統一**する。
>
> **現時点での読み方**: API 仕様（R1〜R10、sCLMID、EVENT 規約、Shift-JIS、URL 形式）は一次資料として信頼してよい。ファイルパス参照 / env 名 / Rust ヘルパー名は [docs/plan/tachibana/](../../../docs/plan/tachibana/) を正として読み、本ファイルは T0 でその計画に合わせて書き換える。

flowsurface では `exchange/src/adapter/tachibana.rs` で立花証券 e支店 API を扱う（**予定**、計画フェーズ）。本スキルは、Claude が API 仕様に正しく沿ってコーディングするためのルール集である。
...
`DEV_USER_ID` / `DEV_PASSWORD` / `DEV_SECOND_PASSWORD` / `DEV_IS_DEMO` による自動ログインは **`#[cfg(debug_assertions)]` ブロックのみで有効**（[`src/screen/login.rs:140`](../../../src/screen/login.rs#L140), [`src/connector/auth.rs:91-100`](../../../src/connector/auth.rs#L91)）。
...
```
DEV_USER_ID=...          # 立花ユーザーID
DEV_PASSWORD=...         # ログインパスワード
DEV_SECOND_PASSWORD=...  # 第二暗証番号（発注 API で必要、R10 も参照）
...
意味:
- **初回だけ**: `DEV_USER_ID` / `DEV_PASSWORD` / `DEV_IS_DEMO` が必要（手動の電話認証は別途ユーザーが済ませている前提）
- **2 回目以降**: 仮想 URL が夜間閉局までは keyring セッションで起動できる。env 未設定でも動く
- **keyring を壊したいとき**: `POST /api/auth/tachibana/logout`（メモリ + keyring 両方クリア）。debug ビルドでは `POST /api/test/tachibana/delete-persisted-session` も使える（E2E teardown 用、[`src/replay_api.rs`](../../../src/replay_api.rs) の `TestCommand`）
...
## Rust 実装の既存ヘルパー（[tachibana.rs](../../../exchange/src/adapter/tachibana.rs) 内で定義）

新しい sCLMID を追加する際は下記を踏襲する。新しい送信/パース処理は自前で書かず、既存ヘルパーを再利用する。

- `next_p_no()` — `AtomicU64` + Unix 秒初期化で単調増加 p_no を生成（R4、自前採番禁止）
- `current_p_sd_date()` — JST (UTC+9) 固定の送信日時文字列（R4）
...
- POST 系（注文・訂正・取消）: `post_request(client, url, json_body)` — URL クエリに JSON を載せる GET と body に JSON を載せる POST で同じ API が両対応している（マニュアルは GET 例のみ、サンプル [`e_api_sample_v4r8.py`](samples/e_api_sample_v4r8.py) は POST 版）
- 応答の Shift-JIS デコード: `decode_response_body`（R7）
- EVENT 受信データの制御文字分解: `parse_event_frame(data: &str) -> Vec<(&str, &str)>`
- エラー型: `TachibanaError`（`thiserror`）。新しいエラー分類が必要なときだけバリアントを追加。`anyhow` は `exchange` crate では使わない
- テストは `mockito::Server` でモック（`exchange` の dev-dependency）。本番 URL を絶対に踏まない（R1）。ログイン応答は [`samples/e_api_login_tel.py/e_api_login_response.txt`](samples/e_api_login_tel.py/e_api_login_response.txt) の実例を流用する
